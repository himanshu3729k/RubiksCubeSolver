...
public:
    ...
    RubiksCube3dArray() {
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++)
                    cube[i][j][k] = getColorLetter(COLORS(i));
            }
        }
    }

    COLORS getColor(FACES FACES, unsigned row, unsigned col) const {
        char COLORS = cube[int(FACES)][row][col];
        switch (COLORS) {
            case 'B':
                return COLORS::BLUE;
            case 'R':
    ...
    bool isSolved() const override {
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++) {
                    if (this->cube[i][j][k] == getColorLetter(COLORS(i))) continue;
                    return false;
                }
            }
        }
