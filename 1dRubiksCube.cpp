...
private:
    ...
    /*
     * Given a FACES index, row and col, return it's flattened index
     */
    static inline int getIndex(int ind, int row, int col) {
        return (ind * 9) + (row * 3) + col;
    }

    void rotateFACES(int ind) {
        char temp_arr[9] = {};
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
...
public:
    ...
    RubiksCube1dArray() {
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++) {
                    cube[i * 9 + j * 3 + k] = getColorLetter(COLORS(i));
                }
            }
        }
    }

    COLORS getColor(FACES FACES, unsigned row, unsigned col) const override {
        char COLORS = cube[getIndex((int)FACES, (int)row, (int)col)];
        switch (COLORS) {
            case 'B':
                return COLORS::BLUE;
            case 'R':
    ...
    bool isSolved() const override {
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++) {
                    if (this->cube[getIndex(i, j, k)] == getColorLetter(COLORS(i))) continue;
                    return false;
                }
            }
        }
